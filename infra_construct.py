...
def gen_multi_zookeepers_str(zoo_num, curr_index):
    res = ""
    for i in range(zoo_num):
        if i == curr_index:
            res += "0.0.0.0:2888:3888,"
        else:
...
services:"""
    ...
    if zoo_num == 1:
        ...
        for i in range(zoo_num):
            ...
            zookeeper_cur = zookeeper_cur.replace("{net_index}", str(net_index))
            zookeeper_cur = zookeeper_cur.replace("{zookeeper_servers}", gen_multi_zookeepers_str(zoo_num, i))
            res += zookeeper_cur
            net_index += 1
...
if __name__ == "__main__":
    ...
    if shard_num <= 0 or zoo_num <= 0:
        print("sorry! [shard_num must > 0] and [zoo_num must > 0]")
        exit()
